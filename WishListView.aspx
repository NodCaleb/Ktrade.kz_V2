...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
    <title>Ktrade - список желаний</title>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    ...
    <asp:SqlDataSource ID="WishesSource" runat="server" ConnectionString="<%$ ConnectionStrings:KtradeConnectionString %>"
        SelectCommand="
            select
                W.id,
                W.Catalogue_id,
		...
		W.URL,
		W.Fullfilled,
		'http://ktrade.kz/Customer/NewOrder.aspx?catalogue=' + Convert(nvarchar(10), W.Catalogue_id) + '&article=' + W.Article_id + '&name=' + W.ArticleName + '&price=' + Replace(Convert(nvarchar(10), Convert(money, W.Price)), '.', ',') + '&size=' + W.Size + '&color=' + W.Colour + '&URL=' + REPLACE(W.URL, 'http://', '') as OrderURL,
		'http://ktrade.kz/Customer/MyWishList.aspx?catalogue=' + Convert(nvarchar(10), W.Catalogue_id) + '&article=' + W.Article_id + '&name=' + W.ArticleName + '&price=' + Replace(Convert(nvarchar(10), Convert(money, W.Price)), '.', ',') + '&size=' + W.Size + '&color=' + W.Colour + '&URL=' + REPLACE(W.URL, 'http://', '') as WishURL
            from
    ...
    <asp:SqlDataSource ID="ListSource" runat="server" ConnectionString="<%$ ConnectionStrings:KtradeConnectionString %>"
        SelectCommand="
            select top 50
		    C.FirstName + ' ' + C.LastName + ' (' + Convert(nvarchar(10), Count(*)) + ')' as Customer,
		    'http://ktrade.kz/WishListView.aspx?customer=' + Convert(nvarchar(10), C.Customer_id) as URL
	    from
		    Wishes as W with (nolock)
